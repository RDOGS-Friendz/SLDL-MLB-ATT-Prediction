...
st.set_page_config(page_icon="⚾", page_title="MLP Attendance Predictor")
...
with st.form(key="my_form"):
    ...
    def display_dial(title, value, color):
        st.markdown(
            div(
                ...
                h2(style=styles(font_size=rem(0.8),
    ...
    def munge_the_numbers(
        ...
        SentimentListItem = namedtuple(
            "SentimentListItem", ("date", "polarity",
                                  "subjectivity", "text", "url")
        )
        ...
        def to_df(the_dict):
            ...
            items = ((term, count, len(term.split(" ")))
    ...
    search_params["min_replies"] = a.number_input(
        "Minimum replies", 0, None, 0)
    search_params["min_retweets"] = b.number_input(
        "Minimum retweets", 0, None, 0)
    search_params["min_faves"] = c.number_input("Minimum hearts", 0, None, 0)
    selected_rel_date = d.selectbox(
...
results = munge_the_numbers(
    tweets, tweets[0].created_at, tweets[-1].created_at)
...
avg_polarity = chart.mark_line(interpolate="catmull-rom", tooltip=True,).encode(
    x=alt.X("date:T", timeUnit=timeUnit, title="date"),
    y=alt.Y("mean(polarity):Q", title="polarity",
...
st.altair_chart(avg_subjectivity + subjectivity_values,
